...
export default function ArchivePage() {
  ...
  return (
    <ArchiveShell><main className="min-h-screen bg-background text-foreground">
      <section className="border-b border-foreground/10 px-5 pb-10 pt-12 lg:px-8 lg:pt-20">
        <div className="mx-auto max-w-7xl">
          <div className="flex items-end justify-between gap-4"><div><p className="mb-3 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-accent">The reading room</p><h1 className="text-5xl font-semibold tracking-[-0.06em] sm:text-7xl">Digital Archive</h1></div><span className="hidden font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground sm:block">{manuscripts.length} records · {topicCount} categories</span></div>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">Explore the writings, speeches, and manuscripts of Dr. B. R. Ambedkar.</p>
          <div className="mt-7 flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><span className="sr-only">Search archive</span><Search size={16} className="absolute left-3 top-3.5 text-muted-foreground" /><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search manuscripts, speeches, essays..." className="w-full border border-foreground/20 bg-card py-2.5 pl-10 pr-4 text-sm outline-none transition-colors focus:border-primary" /></label><div className="flex flex-wrap gap-2">{categories.map((category) => <button key={category} onClick={() => setSelectedCategory(category)} className={`border px-3 py-2 text-xs font-semibold transition-colors ${selectedCategory === category ? 'border-primary bg-primary text-primary-foreground' : 'border-foreground/20 bg-card hover:border-primary'}`}>{category}</button>)}</div></div>
        </div>
      </section>
